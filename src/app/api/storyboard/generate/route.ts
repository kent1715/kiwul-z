...
export async function POST(request: NextRequest) {
  try {
    ...
    // Delete existing storyboards and scenes for this project
    const existingStoryboards = await db.storyboard.findMany({ where: { project_id } })
    for (const sb of existingStoryboards) {
      await db.scene.deleteMany({ where: { storyboard_id: sb.id } })
    }
    await db.storyboard.deleteMany({ where: { project_id } })
    ...
    await db.scene.createMany({ data: scenesData })
    ...
    return NextResponse.json({ storyboard: result, scenes: result?.scenes || [] }, { status: 201 })
  } catch (error) {
