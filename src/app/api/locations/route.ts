...
export async function POST(request: NextRequest) {
  try {
    ...
    const { project_id, generate } = body
    ...
    // If generate flag is set, use LLM to generate locations
    if (generate) {
    ...
    // Otherwise, create a single location from provided data
    const { name, description, lighting, camera_style, consistency_prompt } = body
    ...
    const location = await db.location.create({
      data: {
        ...
        camera_style: camera_style || null,
        consistency_prompt: consistency_prompt || null,
      },
    })

    return NextResponse.json(location, { status: 201 })
  } catch (error) {
    console.error('Error creating/generating locations:', error)
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Failed to create/generate locations' },
      { status: 500 }
    )
  }
}
